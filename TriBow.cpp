...
void TriBow::UseSkill(const DIRECTION _AmingDir)
{
	...
	if (_AmingDir == DIRECTION::DOWN || _AmingDir == DIRECTION::UP)
	{
		...
		ArrowStorm::GetInstance().GetProjectileList().emplace_back(std::make_unique<Projectile>(m_Owner, InitialPos + Position(1, 0), Object, _AmingDir, m_Damage));
		ArrowStorm::GetInstance().GetProjectileList().emplace_back(std::make_unique<Projectile>(m_Owner, InitialPos + Position(-1, 0), Object, _AmingDir, m_Damage));
	}
	else
	{
		ArrowStorm::GetInstance().GetProjectileList().emplace_back(std::make_unique<Projectile>(m_Owner, InitialPos, Object, _AmingDir, m_Damage));
		ArrowStorm::GetInstance().GetProjectileList().emplace_back(std::make_unique<Projectile>(m_Owner, InitialPos + Position(0, 1), Object, _AmingDir, m_Damage));
		ArrowStorm::GetInstance().GetProjectileList().emplace_back(std::make_unique<Projectile>(m_Owner, InitialPos + Position(0, -1), Object, _AmingDir, m_Damage));
	}
}